...
_BaseDashboardHandler = _DashboardHandler
_BaseDashboardServer = DashboardServer


class _DashboardHandler(_BaseDashboardHandler):
    def _current_profile(self) -> Dict[str, Any]:
        profile = super()._current_profile()
        if profile:
            return profile
        if str(__import__("os").environ.get("ROCKETCOACH_DEV_BYPASS_AUTH", "")).strip() != "1":
            return {}
        if self.db is None:
            return {}
        auth = self.db.get_auth_user_by_email(email="brevintating1@gmail.com")
        if not auth:
            auth = self.db.create_auth_user(email="brevintating1@gmail.com", password="dev-bypass-only")
        profile = self.db.get_profile_by_auth_user(auth_user_id=int(auth["id"])) or {}
        if not profile:
...
class DashboardServer(_BaseDashboardServer):
    def start(self) -> None:
        handler = type("RocketCoachDashboardHandler", (_DashboardHandler,), {})
        handler.store = self.store
        handler.review_store = self.review_store
        handler.web_dir = self.web_dir
        handler.collision_mesh_dir = self.collision_mesh_dir
