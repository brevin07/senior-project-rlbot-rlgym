...
from rocketcoach.extract_player_data import CSV_DIR, _resolve_rrrocket_path, _run_rrrocket_to_json, extract_final
from rocketcoach.live_analysis.future_event_engine import apply_whiff_rate_from_events, refine_events_posthoc
from rocketcoach.live_analysis.metrics_engine import LiveMetricsEngine
from rocketcoach.replay_dashboard.replay_packet_adapter import ReplayPacketBuilder
...
def load_replay_bytes(file_name: str, data: bytes, rrrocket_override: str | None = None) -> ReplaySession:
    ...
    with tempfile.TemporaryDirectory(prefix="rl_replay_dash_") as td:
        ...
        t_extract = datetime.utcnow().timestamp()
        with contextlib.redirect_stdout(io.StringIO()):
            extract_final(str(json_path), out_csv_name)
        print(f"[replay_perf] extract_final_csv {max(0.0, datetime.utcnow().timestamp() - t_extract):.3f}s")

        csv_path = Path(CSV_DIR) / out_csv_name
        if not csv_path.exists():
            raise RuntimeError("CSV extraction failed.")

        t_read = datetime.utcnow().timestamp()
        df = pd.read_csv(csv_path)
        print(f"[replay_perf] pandas_read_csv {max(0.0, datetime.utcnow().timestamp() - t_read):.3f}s")
        t_meta = datetime.utcnow().timestamp()
        ...
        print(f"[replay_perf] replay_meta_extract {max(0.0, datetime.utcnow().timestamp() - t_meta):.3f}s")
    ...
    t_state = datetime.utcnow().timestamp()
    df = _annotate_df_game_state(df, replay_meta)
    print(f"[replay_perf] annotate_game_state {max(0.0, datetime.utcnow().timestamp() - t_state):.3f}s")
    t_timeline = datetime.utcnow().timestamp()
    timeline = _build_timeline(df, players)
    print(f"[replay_perf] build_timeline {max(0.0, datetime.utcnow().timestamp() - t_timeline):.3f}s")
